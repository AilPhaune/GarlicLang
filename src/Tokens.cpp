...
GLexingResult::GLexingResult() : tokens({}), error(nullptr) {};
...
size_t GLexingResult::size() {
	return this->tokens.size();
}
std::shared_ptr<GToken> GLexingResult::tokenAt(size_t idx) {
	return this->tokens.at(idx)->getToken();
}
std::shared_ptr<GarlicError> GLexingResult::getError() {
	return this->error;
}
std::vector<std::shared_ptr<GToken>> GLexingResult::getTokens() {
	std::vector<std::shared_ptr<GToken>> res(this->tokens.size());
	for (uint64_t i = 0; i < res.size(); i++) {
		res[i] = this->tokens[i]->getToken();
	}
	return res;
}

GLexer::GLexer(std::string file, const char* content, size_t contentlength): file(file), content(content), contentlength(contentlength) {
	this->idx = 0;
	this->line = 0;
	this->linePos = 0;
}
...
bool GLexer::eof() {
	return this->contentlength <= this->idx;
}
...
bool GLexer::matchExpr(std::string expr) {
	if(this->contentlength - this->idx < expr.size()) {
		return false;
	}
	return std::memcmp(this->content + this->idx, expr.c_str(), expr.size()) == 0;
...
bool GLexer::matchRegex(std::regex expr) {
	return std::regex_match(this->content + this->idx, expr);
}
...
gotEOF:
	if(res->size() == 0 || res->tokenAt(res->size() - 1)->type != GTokenType::END_OF_FILE) {
		std::shared_ptr<GTokenResult> tokres = std::shared_ptr<GTokenResult>(new GTokenResult());
		res->push(tokres->success(std::shared_ptr<GToken>(new GToken(GTokenType::END_OF_FILE, "EOF", this->pos()))));
	}
	return res;
}
std::shared_ptr<GLexingResult> GLexer::makeRawTokens() {
	std::shared_ptr<GLexingResult> res = std::shared_ptr<GLexingResult>(new GLexingResult());
	while (true) {
		if (this->eof()) {
			goto gotEOF;
