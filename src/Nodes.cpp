...
#define PIPE_TOP_RIGHT "└"
#define PIPE_VERTICAL_RIGHT "├"
...
std::string GNode::toString(std::shared_ptr<PrettyPrint> map, uint32_t spaces) {
	...
	while (it != map->children.end()) {
		...
		ret << std::endl << PIPE_VERTICAL << std::endl << (isNotLast ? PIPE_VERTICAL_RIGHT: PIPE_TOP_RIGHT);
		...
		while ((next = child.find("\n", last)) != std::string::npos) {
			if (pos == 0) {
				...
				ret << std::endl << (isNotLast ? PIPE_VERTICAL : " ") << spaces_;
			}
